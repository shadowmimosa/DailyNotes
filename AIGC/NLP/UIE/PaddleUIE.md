_该项目已在 [PR #9311: change the legacy to slm](https://github.com/PaddlePaddle/PaddleNLP/pull/9311) 中移到 `slm` 目录，目测重心转向 `llm` 不再维护了。_
...
0-shot 表示无训练数据直接通过 `paddlenlp.Taskflow`进行预测，5-shot 表示每个类别包含 5 条标注数据进行模型微调。**实验表明 UIE 在垂类场景可以通过少量数据（few-shot）进一步提升效果**。
